...
public class ReservationCancelAction implements Reservation {
	...
	public ReservationForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		*/
		...
		int rez_num = Integer.parseInt(request.getParameter("rez_num")); //ReservationCancel.jsp에서 주소줄로 보낸 후 파람으로 가져오기
		...
		rezDTO.setPay_uqNum(Integer.parseInt(request.getParameter("pay_uqNum")));
//		rezDTO.setCancel_date(request.getParameter("cancel_date")); //*************************타임스탬프 어떻게 쓰는지 제대로 알아내자...
		...
		// DAO 객체 생성 - 예약취소 정보저장 메서드 호출
		rezDAO.reservationCancelChange(rezDTO);
